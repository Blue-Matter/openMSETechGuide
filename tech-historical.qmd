...
The historical period spans `OM@nYear` years divided into `OM@nYear × OM@Seasons` time steps, each of duration $1/\text{Seasons}$ years. For non-seasonal models (`OM@Seasons = 1`) each time step is one year. Mortality rates (M and F) are expressed in the same units as the time step. 

The simulation index is omitted from equations below; all calculations are performed independently per simulation.
...
where $q_{\text{st},f,t}$ is catchability for stock $\text{st}$ by fleet $f$, $v_{a,f,k,t}$ is selectivity, and $r_{a,f,k,t}$ is retention. 

By default (`Effort@Mode = "Density"`), $\hat{B}_{f,k,t}$ is divided by the relative area size $s_k$ so that effort is attracted by fish density rather than total abundance in each area, appropriate for non-aggregating species where catchability per unit effort depends on local density, not area-wide biomass. Set `Effort@Mode = "Biomass"` for schooling or aggregating species where total biomass in an area drives fleet behaviour regardless of area size.

The resulting exploitable biomass (or density) signal gives the initial effort allocation based on the ideal free distribution (IFD):
...
The second term accounts for inter-fleet competition: pressure from other fleets is included using their effort distribution from the previous time step as a one-step lag. For biomass-mode fleets the $s_k$ denominator is omitted. 

The depletion-adjusted utility for fleet $f$ in area $k$ is:
...
The current model uses exploitable biomass as the utility signal, implicitly assuming that the value of catch is proportional to vulnerable biomass (i.e., uniform price across stocks) and that costs are uniform across areas. A future version will incorporate stock-specific prices and area-specific costs so that the utility signal reflects expected profit per unit effort as part of the bio-economic model development.
:::
...
for density-mode fleets (`Effort@Mode = "Density"`). For biomass-mode fleets (`Effort@Mode = "Biomass"`) the $s_k$ denominator is omitted. $F_{\max}$ (`OM@maxF`) is a cap preventing unrealistically large fishing mortalities. Three mortality components are derived per fleet:

1. $F^{\text{ret}}_{a,f,k,t} = F^{\text{int}}_{a,f,k,t} \cdot r_{a,f,k,t}$: retained (landed) mortality
2. $F^{\text{dis}}_{a,f,k,t} = F^{\text{int}}_{a,f,k,t} \cdot (1 - r_{a,f,k,t}) \cdot m_{a,f,k,t}$: discarded and dead
3. $F^{\text{dead}}_{a,f,k,t} = F^{\text{ret}}_{a,f,k,t} + F^{\text{dis}}_{a,f,k,t}$: total fishing mortality

where $m_{a,f,k,t}$ is discard mortality-at-age (see @sec-tech-fleet-schedules). 

Total mortality combining natural and fishing mortality is:

$$Z_{a,k,t} = M_{a,t} + \sum_f F^{\text{dead}}_{a,f,k,t}$$

The apical value for each fleet is computed by first taking the numbers-weighted average of F across areas at each age, then taking the maximum over ages:
...
For a two-area model with staying probabilities $p_1$ and $p_2$ on the diagonal, the off-diagonal elements are $1 - p_1$ and $1 - p_2$. The staying probability for area 2 is solved analytically from the target $d_1$ and the supplied $p_1$:
...
If an age-weight key is available instead of an age-length key, the same approach is applied to give catch-at-weight. Results are stored in `Hist@LandingsAtSize` and `Hist@DiscardsAtSize` as nested lists (by stock, then fleet), each array with dimensions `Sim × Class × Year × Area`. 