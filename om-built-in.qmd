# Built-In Examples {#sec-om-built-in}
...
The code block below demonstrates the resulting selectivity schedules when this `Selectivity` object is paired with the `AlbacoreExStock` stock object.

It first builds a small `OM` combining `AlbacoreExStock` and `AsympExFleet` (with `nSim = 5` to keep the figure legible), then uses `PlotSelectivity()` and `PlotMaturity()` — part of `openMSE`'s built-in family of schedule-plotting functions, see `?plot_schedule` — to show the resulting curves at both length and age.

In normal use, populating an `OM` happens automatically when `Populate()` is called; it is done explicitly here for illustration only. See @sec-populate for a full description of the population process.

```{r fig-asymp-selectivity}
#| fig-cap: "Selectivity (top row) and maturity (bottom row) schedules for `AsympExFleet` paired with `AlbacoreExStock`, shown at length (left) and age (right), produced by `PlotSelectivity()` and `PlotMaturity()`."
```
...
#| echo: false
#| fig-cap: "Selectivity (top row) and maturity (bottom row) schedules for `DomeExFleet` paired with `AlbacoreExStock`, shown at length (left) and age (right), produced by `PlotSelectivity()` and `PlotMaturity()`."
```
...
The `Obs` object interacts closely with the corresponding `Data` object. See @sec-objectdata for the `Data` object's structure and @sec-obsdataconditioning for how the two interact.
...
## Example `Imp` Objects {#sec-exampleimp}

An `Imp` object defines the implementation error model, controlling how management recommendations are imperfectly applied in the operating model.

See `?Imp` for full documentation, and `?ImpSlot` for documentation of the underlying `TAC`/`Effort`/`Size` subcomponent.

Four example `Imp` objects are provided, representing different assumptions about how management advice is realised in practice:

- **`FullComplianceImp`**: TAC and effort advice are realised exactly, and size regulations are adopted immediately by the whole fleet.
- **`OverageImp`**: TAC and effort are consistently realised above advice.
- **`UnderageImp`**: TAC and effort are consistently realised below advice.
- **`PartialSizeComplianceImp`**: TAC and effort are implemented perfectly, but only part of the fleet adopts a newly-advised size regulation.

### `FullComplianceImp`

```{r}
#| include: false
knitr::read_chunk("../MSEtool/data-raw/ex-imp-full-compliance.R")
```

`FullComplianceImp` represents perfect implementation of management advice: TAC and effort recommendations are realised exactly, and any size-based regulation is adopted immediately by the entire fleet.

```{r full-compliance-create-imp}
```

Because this is behaviourally identical to leaving `Imp` unspecified on an `OM` object, `FullComplianceImp` simply makes that default explicit. It is the `Imp` object assigned to every other built-in example `OM` (e.g., `SingleStockOM`, `TwoFleetOM`).

### `OverageImp`

```{r}
#| include: false
knitr::read_chunk("../MSEtool/data-raw/ex-imp-overage.R")
```

`OverageImp` represents a fleet that consistently realises more catch and effort than advised, e.g. under-reporting, unrecorded discards landed elsewhere, or effort creep beyond the recommended level.

```{r overage-create-imp}
```

TAC and effort are realised on average 5-25% above the advised level (`Mean` 1.05-1.25), with additional simulation-to-simulation and year-to-year stochastic variation (`SD` 0.05-0.15). `Size` is left empty: this example isolates TAC/effort overage.

### `UnderageImp`

```{r}
#| include: false
knitr::read_chunk("../MSEtool/data-raw/ex-imp-underage.R")
```

`UnderageImp` represents a fleet that consistently realises less catch and effort than advised, e.g. quota left unfilled because the fishery isn't profitable enough to fully prosecute, or an effort target that isn't reached due to weather, capacity, or market constraints.

```{r underage-create-imp}
```

TAC and effort are realised on average 10-30% below the advised level (`Mean` 0.7-0.9), with the same `SD` range as `OverageImp`. `Size` is left empty.

### `PartialSizeComplianceImp`

```{r}
#| include: false
knitr::read_chunk("../MSEtool/data-raw/ex-imp-partial-size-compliance.R")
```

`PartialSizeComplianceImp` isolates partial fleet adoption of a size-based regulation, while TAC and effort advice are still implemented perfectly.

```{r partial-size-compliance-create-imp}
```

`TAC` and `Effort` use `Mean = 1`, `SD = 0`, as in `FullComplianceImp`. Only 60% of the fleet adopts a newly-advised size regulation in the year it changes (`Compliance = 0.6`); the remaining 40% continues fishing under the previous Retention/Selectivity curve.
