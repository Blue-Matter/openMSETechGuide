...
- **Equilibrium** (`Hist@Unfished@Equilibrium`): the steady-state population produced by `R0`, current natural mortality, and (for seasonal models) the seasonal pattern of recruitment, with no fishing. 
- **Dynamic** (`Hist@Unfished@Dynamic`): the population trajectory that would have occurred over the historical period had fishing never taken place (i.e.\ catchability set to zero). 
...
For an annual model, survivorship-at-age is:
...
When `OM@Seasons > 1`, each season has its own recruitment and natural mortality values, so equilibrium numbers-at-age cannot be expressed in closed form. Instead, `CalcUnfishedNumber()` iterates the seasonal cycle (applying recruitment, mortality, and ageing each season) until the numbers-at-age converge to a stable seasonal pattern. Within each season $s$, recruitment enters the first age class and survivors age up one class:
...
where $\bar{W}_{a,t}$ is mean weight-at-age, $\text{mat}_{a,t}$ is maturity-at-age, and $f_{a,t}$ is fecundity-at-age (see @sec-tech-fecundity). Unfished spawning production per recruit is:

$$\phi^0_t = \frac{\mathit{SP}^0_t}{R_{0,t}}$$

$\phi^0$ appears in the stock–recruitment parameterisations (see @sec-tech-srr) and in the per-recruit reference point calculations (see @sec-tech-refpoints).
...
Spatial dynamics during the historical period are developed in @sec-tech-historical.
